...
def find_neighbor(maze, work_path, len_new_path, starting_index, extend=False):
...
def tabu_search(maze, start, goal, len_intermediate_paths=4, iteration_max=5, max_total_iterations =100):
    path = find_intital_path(maze, start, goal)
    # initialize memory vector
    memory_list = [0 for _ in path[0:len(path) - len_intermediate_paths - 1]]
    cost = path_cost(maze, path)
    t = 0
    while cost > len(maze) + len(maze) and t <= max_total_iterations:
        non_tabu_solutions = []
        asperation_by_influence = True
        for i in range(len(memory_list)):
            if t-memory_list[i] > iteration_max:
                neighbor_path = find_neighbor(maze, path, len_intermediate_paths, i)
                neighbor_path_cost = path_cost(maze, neighbor_path)
                non_tabu_solutions.append((neighbor_path, neighbor_path_cost, i))
                # new solution is expected to be at least 20% better than the old one
                if neighbor_path_cost < 0.8*cost:
                    path = neighbor_path
                    cost = neighbor_path_cost
                    memory_list[i] = t
                    asperation_by_influence = False
                    break
        activate_long_term_memory = True
        tabu_solutions = []
        if asperation_by_influence:
            for i in range(len(memory_list)):
                if t-memory_list[i] <= iteration_max:
                    neighbor_path = find_neighbor(maze, path, len_intermediate_paths, i)
                    neighbor_path_cost = path_cost(maze, neighbor_path)
                    tabu_solutions.append((neighbor_path, neighbor_path_cost))
                    # new solution is expected to be at least 20% better than the old one
                    if neighbor_path_cost < 0.8 * cost:
                        path = neighbor_path
                        cost = neighbor_path_cost
                        memory_list[i] = t
                        activate_long_term_memory = False
        # Asperation by default
        if not non_tabu_solutions:
            # choose the oldest move from memory
            oldestVal = t
            oldest_index = 0
            for i in range(len(memory_list)):
                if memory_list[i] < oldestVal:
                    oldest_index = i
            path = tabu_solutions[oldest_index][0]
            cost = tabu_solutions[oldest_index][1]
            activate_long_term_memory = False
        min_long_term_mem_values = {}
        if activate_long_term_memory:
            #choose the best of the non-tabu moves
            for move in non_tabu_solutions:
                (ltpath, ltcost, ltindex) = move
                val = ltcost*0.3 + memory_list[ltindex]*0.7
                min_long_term_mem_values.update({val: move})
            (final_path, final_cost, final_index) = min_long_term_mem_values[min(list(min_long_term_mem_values.keys()))]
            path = final_path
            cost = final_cost
        t += 1
    print(cost)
...
def main():
    ...
    tabu_search(maze, starting_node, goal_node)
